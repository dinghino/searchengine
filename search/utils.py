...
def max_distance(phrase, word_index):
    ...
    Arguments:
        ...
        word_index(int): integer representing the index of the position to
            count from. The value ** must ** be `>= 0` but ** can ** be over
...
def position_similarity(token1, token2, phrase1, phrase2):
    """
    Get the normalized inverted movement cost for for between `token1` and
    `token2` on the `phrase2` iterable.
    ...
    Moves are relative to token1 on phrase1, which `should` be the longest set
    ...
    Arguments:
        ...
        phrase1 (iterable): iterable allowing the ``index`` method, should be
            the longest of the two iterables.
        phrase2 (iterable): iterable allowing the ``index`` method, should be
            the shortest of the two iterables.
